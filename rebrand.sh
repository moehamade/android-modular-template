...
# Pre-flight check: Warn if git repo has uncommitted changes
if git rev-parse --git-dir > /dev/null 2>&1; then
    if ! git diff-index --quiet HEAD -- 2>/dev/null; then
        print_warning "You have uncommitted changes in your git repository!"
        print_info "It's recommended to commit or stash them first."
        echo ""
        read -p "Continue anyway? [y/N]: " continue_dirty
        if [[ ! "$continue_dirty" =~ ^[Yy]$ ]]; then
            print_info "Rebranding cancelled. Commit your changes and try again."
            exit 0
        fi
        echo ""
    fi
else
    print_warning "Not a git repository. Changes cannot be easily reverted."
    echo ""
fi
...
echo ""
print_info "Optional: Start fresh with new git history"
echo "  rm -rf .git && git init && git add . && git commit -m 'Initial commit: $NEW_PROJECT_NAME'"
echo ""
